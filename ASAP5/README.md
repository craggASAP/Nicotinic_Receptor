...
For the **Voltage_sensor_spontaneous_nAChR_event_detection.ipynb** and **Voltage_Sensor_analysis_nAChR_mediated_depolarisation_metrics.ipynb**, data will be extracted directly from the raw excel files organised with column headers as recording number & electrical stimulation applied for that recording, with below the raw fluorescence extracted from ImageJ.
