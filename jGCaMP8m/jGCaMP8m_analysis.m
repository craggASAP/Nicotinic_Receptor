...
exposure=1/993.05; % exposure every 0.917 ms (frequency of recording around 1000 Hz)- to change if the number of frame per seconds changes
...
for i=2:size(A,2); % size(A,2) is the amount of transients you have in your experiment
    ...
    % The linear fit is taken to match 500 frames (~0.50 seconds) before stim onset
    % stim onset at 1.4 seconds ~ frame 1390
    Fit=fit(A(880:1380,1),raw_F(880:1380,:), 'poly1'); % fit is a*x+b = rawF, adjusting the a and b coefs
    ...
    % Corrects for baseline by calclating F-F0 
    ...
    % calculate the total AUC and peak
    total_AUC_all(:,i-1)=trapz(deltaF_over_F0_percent(1390:3000)); %1390 frame is when the first pulse happens, and 3000 frames (around 1.6s later) seems to be when there is a return to "baseline" for 1p outside of DHBE
    Peak_all(:,i-1)=max(deltaF_over_F0_percent(1390:1450));
...
Traces_all= cat(2,frame_number*exposure,deltaF_over_F0_percent_all);
Peak_AUC_F0 = cat(1,Peak_all,total_AUC_all,F0_all);
